...
fn main() {
    ...
    while let Some(arg) = args.next() {
        match arg.as_ref() {
            ...
            "--help" => {
                println!(
                    r#"Run 'rockhttp' in a folder to host that folder.
...
Examples:
rockhttp --address 127.0.0.1:8080 --path "some_directory/subdirectory" --header Access-Control-Allow-Origin='*'
                "#
                );
                return;
            }
            _ => {
                println!(
                    "Unrecognized flag: `{:?}`.\nSee available options with `rockhttp --help`",
                    arg
                );
                return;
            }
